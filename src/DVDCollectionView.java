...
public class DVDCollectionView extends Pane{
    ...
    public void update(DVDCollection model){

        ObservableList<String> listOfTitles = FXCollections.observableArrayList();
        ...
        for (int i = 0; i < model.getDvdCount(); i++){
            DVD select = model.getDVDList().get(i);
            listOfTitles.add(select.getTitle());
            listOfYears.add(select.getYear());
            listOfLengths.add(select.getDuration());
        }
        tList.setItems(listOfTitles);
        yList.setItems(listOfYears);
        lList.setItems(listOfLengths);
